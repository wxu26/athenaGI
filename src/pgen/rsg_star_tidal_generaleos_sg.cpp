...
static Real den0, p0_over_r0, mu, midprat, gunit, gcode;
...
void Mesh::InitUserMeshData(ParameterInput *pin) 
{
  ...
  //get radiation-related parameters
   // if(NR_RADIATION_ENABLED|| IM_RADIATION_ENABLED){
        ...
        gunit = (8.31447e7*tunit/mu)/lunit/lunit/rhounit;
        gcode = 6.67e-8/gunit;
...
//                             /(ini_profile(lright,0) - ini_profile(lleft,0));
  ...
  for(int i=is; i<=ie; ++i) {
    ...
    if(x1 > rmax){
      ...
      rhostar = std::max(rhostar,rhofloor);
 ...
 //   rho *= (1.0 + amp * ((double)rand()/(double)RAND_MAX-0.5));
      ...
      for (int j=js; j<=je; ++j) {
        ...
        Real den = den0*zfactor*zfactor*zfactor; //disk density
          
            if (den < rhofloor) {den = rhofloor;}
          
        Real disktemp = p0_over_r0*zfactor; //disk temperature
          
            if (disktemp < tfloor) {disktemp = tfloor;}
        
        for (int k=ks; k<=ke; ++k) {
            Real rho = std::max(den, rhostar); //larger of density
            Real tem = std::max(temstar, disktemp);//larger of temperature
            ...
            if (NON_BAROTROPIC_EOS){
              ...
              phydro->u(IEN,k,j,i) = 1.5 * tem * rho + 0.5486219083605187 * tem * tem * tem * tem; //general equation of state
